...
/**
 * The two key purposes this table holds (ATL-200, ATL-203).
 *
 * `content` — AES-256-GCM DEK; original behavior; all rows created before
 * ATL-200 carry this purpose via the migration DEFAULT.
 *
 * `rejection` — 256-bit HMAC-SHA256 key for rejection fingerprints (ATL-203,
 * ADR-008 §8). Never used as an AES-GCM key.
 *
 * Additional purposes require a new or amended ADR.
 */
export type KeyPurpose = "content" | "rejection";
...
function toRecord(row: KeyRow): EncryptionKeyRecord {
  return {
    ...
    // Pre-ATL-200 rows carry no key_purpose column; the migration DEFAULT ensures
    // they are 'content' in the database, but a stale generated type could omit
    // the field. The fallback here is belt-and-suspenders.
    keyPurpose: (row.key_purpose ?? "content") as KeyPurpose,
  };
}
...
export class EncryptionKeyRepository {
  ...
  /**
   * Every key row the user holds, in any status and any purpose.
   *
   ...
   * rows itself.
   *
   * A user accumulates one active key per purpose plus a bounded number of
   * retired or destroyed ones, so this stays a single small read.
   */
  ...
  /**
   * All rows for a user scoped to one key purpose.
   *
   * Used by purpose-specific services (e.g. `RejectionKeyService`) whose
   * `keyState` classification must not cross purpose boundaries. A destroyed
   * content key must not read as a destroyed rejection key and vice versa.
   */
  async findForUserByPurpose(
    userId: string,
    keyPurpose: KeyPurpose,
  ...
  /**
   * Inserts a new active content key (key_purpose = 'content').
   *
   ...
   *
   * The database DEFAULT `key_purpose = 'content'` is relied upon here. For
   * other purposes use `insertActiveForPurpose`.
   */
  async insertActive(
    userId: string,
    wrappedDek: string,
  ...
  /**
   * Inserts a new active key with a caller-specified id and purpose.
   *
   * The `id` is pre-generated by the caller so the wrapping AAD can be bound
   * to the row before it is written (ATL-203). A conflict on the unique partial
   * index means a concurrent winner committed first; returning null tells the
   * caller to re-read.
   */
  async insertActiveForPurpose(
    id: string,
    userId: string,
    wrappedKey: string,
    kekVersion: number,
    ...
    // Unique index violation = race; null signals re-read.
    if (error) return null;
    return data ? toRecord(data) : null;
  }
  ...
  /**
   * Every key of a specific purpose still holding material, for a
   * purpose-scoped KEK rotation sweep (ATL-203).
   *
   * Content and rejection keys use different wrapping AADs, so a rotation
   * sweep must not mix purposes: re-wrapping a rejection key through the
   * content-key AAD (or vice versa) will fail authentication.
   *
   * Idempotent and resumable for the same reasons as `listWrappedUnder`.
   */
  async listWrappedUnderByPurpose(
    keyPurpose: KeyPurpose,
    kekVersion: number,
    limit: number,
  ): Promise<EncryptionKeyRecord[]> {
  ...
  /**
   * Crypto-shredding: destroys every key the user holds, across all purposes.
   *
   ...
   *
   * Intentionally purpose-agnostic: account deletion must shred both content
   * and rejection keys in one pass (ATL-203, ATL-082). A purpose filter here
   * would silently leave one key class intact.
   */
