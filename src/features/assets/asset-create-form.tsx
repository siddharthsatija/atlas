...
/**
 * Add a service (ATL-032, frontend §6 "Add asset button").
 *
...
export interface AssetCreateFormProps {
  action: (state: AssetCreateFormState, formData: FormData) => Promise<AssetCreateFormState>;
  initialState: AssetCreateFormState;
}
...
export function AssetCreateForm({ action, initialState }: AssetCreateFormProps) {
  ...
  return (
    ...
    >
      {state.failure && (
        /**
         * The test id identifies *this* banner, which exists only when the save
         * failed. `role="alert"` is what announces it, but it is not a usable
         * handle: the shell renders a live region of the same role on every
         * page, so a role query matches something permanent and empty and says
         * a save failed when nothing did.
         */
        <p
          data-testid="create-asset-error"
          role="alert"
      ...
      <div className="flex flex-col gap-1">
        ...
        <Input
          ...
          id="serviceDomain"
          name="serviceDomain"
          defaultValue={state.values.serviceDomain ?? ""}
