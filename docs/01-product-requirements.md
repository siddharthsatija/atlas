...
Atlas is discovery-first. Users build an Identity Profile — the personal fields they explicitly provide to Atlas — and Atlas uses those fields to search approved evidence sources for digital relationships they may not have recalled or recognised. Discovery coverage is bounded: Atlas searches the evidence sources and providers it has access to (at Phase 1: GitHub active for username-enumeration discovery; HIBP breach-corpus infrastructure is accepted but parked by ATL-216 and is not part of the active Phase 1 discovery run; connected sources are a separately gated future capability) and builds the best supported picture it can from retrievable evidence. No relationship is proposed without evidence, and every proposed candidate requires the user's confirmation before it becomes part of their confirmed digital identity map.
...
**Mission:** Give every person meaningful visibility and meaningful control over their digital presence. Discovery coverage is bounded by available providers and evidence sources; Atlas does not guarantee or imply comprehensive coverage of a user's full digital footprint.
...
Atlas's visual design uses a light palette as its baseline. Atlas launches in light mode regardless of OS or system preference. Dark mode is supported as an explicit user opt-in capability and must be a fully considered experience, but Atlas is not a dark-mode-first or system-preference-first product. Marketing, onboarding, and default presentation are light-mode.
...
5. Dashboard with a consolidated view of the user's confirmed digital assets and identity map: privacy score, confirmed digital assets, open findings, active requests, insights, assistant, and activity
...
4. User builds their Identity Profile: every field (email addresses, name, phone, location, usernames and handles) is individually optional. No specific field type is a universal discovery prerequisite. To initiate a real discovery run, Atlas requires at least one user-authorized field that is eligible for at least one currently active provider and has `include_in_discovery` enabled, plus the applicable consent/disclosure requirements for that provider class. If no such eligible active-provider signal exists, Atlas will not pretend a discovery run occurred.
...
6. User grants discovery consent for the applicable active provider classes. Atlas presents disclosure notices only for currently active providers. At Phase 1, GitHub is the active identifying provider; the consent/disclosure flow presents the first-disclosure acknowledgment for each user-supplied handle before it is transmitted to GitHub for the first time. The hashed-query (HIBP) disclosure contract — where a partial hash of the email address, not the plaintext, is transmitted — remains the accepted contract for when HIBP is activated; HIBP is currently parked by ATL-216 and is not part of the Phase 1 initial onboarding discovery run. Broker-search queries require a per-query confirmation at the time the user initiates a broker lookup and are not part of the initial discovery run.
7. Atlas automatically initiates the initial discovery run against active evidence sources using the enabled eligible fields. No separate "Run discovery" or "Start scan" user action is required to begin the initial onboarding run. The run starts as soon as the eligibility and consent conditions of step 4–6 are satisfied.
...
Demo mode is a post-signup feature (OQ-03, resolved). It is not an onboarding branch or escape hatch. Users who decline discovery consent at onboarding continue without discovery active — they are not redirected to demo mode. Manual asset addition is a fallback available throughout the product for discovery misses — it is not an alternative to the discovery-first path at onboarding.

### 9.2 Discovery and candidate adjudication

1. Atlas runs a discovery scan. The initial scan is triggered automatically at onboarding once eligibility and consent conditions are met. Subsequent scans are user-initiated in Phase 1. Scheduled / automated evidence refresh is a Phase 3 capability and must not be built in Phase 1.
...
- Present disclosure notices appropriate to each provider type. Three cases are distinct: (a) for hashed-query providers (HIBP breach corpus), the accepted disclosure contract informs the user that partial hash material derived from their email address — not the plaintext email — is transmitted; this type is not subject to the per-field first-disclosure acknowledgment requirement; at Phase 1 HIBP is parked by ATL-216 and this disclosure flow is not presented until HIBP is activated; (b) for identifying providers (username/handle lookup), display a first-disclosure acknowledgment before the first transmission of each identity field to each named provider; cancelling the notice blocks that invocation only and does not modify the stored field, the `include_in_discovery` preference, or standing consent; a new acknowledgment is required if the provider's disclosure contract changes materially; (c) broker-search queries require a per-query confirmation immediately before each individual search; broker search is not governed by standing discovery consent and is not part of the initial discovery run.
- Capture user goals without requiring sensitive personal data.
- Allow skipping optional steps. Demo mode is a post-signup feature (OQ-03, resolved) — it is not available as an onboarding escape hatch, and declining discovery at onboarding does not route the user to demo mode. Demo mode must not be presented as a co-equal branch alongside discovery-first onboarding.
...
- For identifying providers (username/handle lookup), a first-disclosure acknowledgment is required before the first transmission of each identity field to each named provider. For hashed-query providers, the disclosure contract is distinct (see FR-02 case (a)); per-field acknowledgment does not apply. All disclosure events are recorded and visible in Settings > Discovery (FR-11).
...
- Atlas automatically initiates the initial discovery run once the user has at least one identity field that is eligible for an active provider with `include_in_discovery` enabled, and the applicable consent/disclosure requirements for that provider class are satisfied. No separate "Run discovery" control is required for the initial onboarding run. Subsequent discovery runs may be user-initiated after onboarding; scheduled / automated refresh is a Phase 3 capability.
...
**Zero-eligible-signal and zero-result states.** If the user has no identity field eligible for any currently active provider (e.g., no enabled handle for GitHub), Atlas must explain clearly that discovery cannot run yet, allow the user to add or enable an optional eligible signal, and must not pretend a discovery run occurred. If discovery runs and returns zero candidates, that is a valid zero-result state — not an error. Zero-result copy must not say "your dashboard is empty until you add something" or otherwise prompt manual asset addition as the primary next step. A provider that is blocked, failed, or rate-limited must not be represented as a clean zero-result execution; the discovery run record must distinguish a blocked provider from one that ran and returned no results.
...
> **Phase 2 gate — do not begin until Phase 1 product acceptance clears.** The acceptance environment may be the appropriate integrated environment used for Phase 1 acceptance; a live deployed production environment is not required. Phase 1 acceptance requires all of the following:
> 1. The application path for initial-discovery initiation and orchestration is wired to a real route (DispatchEngine connected; not test-only seeded fixtures).
> 2. The discovery-first onboarding flow matches the approved journey.
> 3. Zero-eligible-signal, clean-zero-result, blocked, failed, and rate-limited states behave correctly.
> 4. Candidate review and adjudication work correctly.
> 5. Confirmed candidates become assets and are reflected correctly in the dashboard.
> 6. Light mode is the default.
> 7. Automated regression and E2E coverage protects the corrected real journey, including production-path initiation (not only seeded downstream candidates).
> 8. The product owner manually walks the complete journey and signs off.
>
> No Phase 2 capability work begins until all eight conditions pass.
