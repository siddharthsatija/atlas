...
- Application-layer envelope encryption for restricted text (full design in ADR-003):
  ...
  - **Encrypted columns (authoritative inventory):** `user_personal_fields.value_encrypted`, `digital_assets.account_identifier_encrypted`, `data_requests.recipient_encrypted`, `data_requests.subject_encrypted`, `data_requests.body_encrypted`, `data_requests.last_status_note`, `ai_messages.content_encrypted`.
  - **`data_requests.last_status_note` was added to this inventory by ATL-056.** Architecture §7.7 listed the column without classifying it, and §3 classifies request recipients, subjects and bodies while saying nothing about notes. It holds what the *service* replied — frontend §9 offers "Add response note", and §13 makes a recorded note one of the two triggers for `sent → awaiting_response` — so it routinely carries case references, agent names and identifiers. Third-party prose about a person is Restricted when it appears in a request body and does not become less so for arriving in the reply. Nothing searches or filters it, so encryption costs the product nothing. The column keeps §7.7's name rather than gaining an `_encrypted` suffix, because §7.7 is the specification a later reader checks; the migration comment and the repository both record that it is an envelope.
