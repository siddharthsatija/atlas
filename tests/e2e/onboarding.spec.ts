 ...
 *
 * Covers what only a browser can show — the seven steps in order, back, skip,
 * the completion redirect, and axe per step.
 */
...
test.describe("the flow", () => {
  test("walks all seven steps and lands on the dashboard", async ({ page }) => {
    ...
    // 6. candidate_review auto-skips for a fresh user with no candidates.
    // 7. Ready.
